...
export function PlatformLoginPage() {
  ...
  const onSubmit = async (data: FullFormData) => {
    ...
    try {
      let discoveredTenant: DiscoveredTenant | null = null

      const lookupResult = await supabase.rpc('lookup_tenant_by_email', {
        lookup_email: data.email,
      })
      const { data: lookupData, error: lookupError } = lookupResult

      if (lookupError) {
        console.error('lookup_tenant_by_email error:', lookupError)
        setLoginError(t('auth.lookupFailed'))
        return
      }

      if (lookupData && typeof lookupData === 'object' && 'slug' in lookupData && 'name' in lookupData) {
        discoveredTenant = lookupData as DiscoveredTenant
      } else if (!demoFeatures.platformConsoleVisible) {
        setLoginError(t('auth.noWorkspaceFound'))
        return
      }
